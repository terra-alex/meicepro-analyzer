  ...
  return (
    <div className="panel p-4">
      <button className="flex items-center justify-between w-full text-left" onClick={() => setOpen((v) => !v)}>
        <h3 className="text-sm font-medium text-white/80">Raw response</h3>
        <span className="text-xs text-white/40">{open ? "hide" : "show"}</span>
      </button>
      {open && (
        <div className="mt-2">
          <label className="text-[11px] text-white/50 flex items-center gap-2 mb-2">
            <input type="checkbox" checked={pretty} onChange={(e) => setPretty(e.target.checked)} />
            pretty-print
          </label>
          <pre className="text-[11px] font-mono leading-snug text-white/80 bg-black/40 rounded p-2 overflow-auto max-h-[480px]">
