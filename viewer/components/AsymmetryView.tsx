  ...
  if (!left || !right) {
    return (
      <div className="panel p-4">
        <h3 className="text-sm font-medium text-[var(--ink)] mb-2">Left vs Right asymmetry</h3>
        <p className="text-xs text-[var(--muted)]">Need both side captures to compute deltas.</p>
      </div>
    );
  }
  ...
  return (
    <div className="panel p-4">
      <div className="flex justify-between items-center mb-3">
        <h3 className="text-sm font-medium text-[var(--ink)]">Left vs Right asymmetry</h3>
        <span className="text-[10px] text-[var(--faint)]">sorted by |Δ|, scores 0..100, higher = better</span>
      </div>
        ...
        {sorted.map((r) => {
          ...
          return (
            <div key={r.key} className="grid grid-cols-[110px_56px_1fr_56px_44px] items-center gap-2">
              ...
              <div className="relative h-2 bg-[var(--surface-alt)] rounded-sm">
                {/* center marker */}
                <div className="absolute left-1/2 top-0 bottom-0 w-px bg-[var(--ink)]/30" />
                <div
              ...
              <span className="text-left font-mono text-[var(--amber)]">{Math.round(r.right)}</span>
              <span className={`font-mono text-right text-[10px] ${Math.abs(r.delta) < 3 ? "text-[var(--faint)]" : isLeftBetter ? "text-violet-300" : "text-[var(--amber)]"}`}>
